...
import { CompletedProtoWallet } from '@bsv/sdk';
...
describe('CurvePoint Library', () => {
    let participants: { wallet: CompletedProtoWallet; publicKey: string }[];
    const message: number[] = [1, 2, 3, 4]; // Example message

    beforeAll(async () => {
        // Create participants with CompletedProtoWallet and public keys
        participants = await Promise.all(
            Array(3)
                .fill(null)
                .map(async (_, index) => {
                    const privateKey = PrivateKey.fromRandom();
                    const wallet = new CompletedProtoWallet(privateKey);
    ...
    test('Handle large group of counterparties', async () => {
        ...
        const walletForRandomParticipant = new CompletedProtoWallet(randomParticipant.privateKey);
    ...
    test('Encryption and Decryption Timing', async () => {
        ...
        for (const groupSize of groupSizes) {
            ...
            for (const messageSize of messageSizes) {
                ...
                console.log(
                    `Encryption time for group size ${groupSize} and message size ${messageSize}: ${endEncryption - startEncryption
                    } ms`
                );

                // Decrypt for one of the participants in the group
                const randomParticipant = group[Math.floor(Math.random() * group.length)];
                const participantWallet = new CompletedProtoWallet(randomParticipant.privateKey); // Use corresponding private key
                ...
                console.log(
                    `Decryption time for group size ${groupSize} and message size ${messageSize}: ${endDecryption - startDecryption
                    } ms`
                );
    ...
    test('Memory Usage', async () => {
        ...
        const randomParticipant = largeGroup[Math.floor(Math.random() * largeGroup.length)];
        const participantWallet = new CompletedProtoWallet(randomParticipant.privateKey);
        const participantCurvePoint = new CurvePoint(participantWallet);
    ...
    test('Cross-Wallet Compatibility', async () => {
        ...
        const participant1 = new CompletedProtoWallet(PrivateKey.fromRandom());
        const participant2 = new CompletedProtoWallet(PrivateKey.fromRandom());
